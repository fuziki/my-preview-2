...
- フォトビューア画面下部、評価・カラーラベルバー（座布団）の右隣にあるPiPボタン（座布団と同じ高さの円）から、現在表示中の写真をシステムの Picture in Picture ウィンドウに表示できる（動画は使わず、`ImagePiPKit` が静止画をシステムPiPへ表示する）。評価機能が無効な場合は座布団があった位置の右端（保存ボタン基準）に配置される。
